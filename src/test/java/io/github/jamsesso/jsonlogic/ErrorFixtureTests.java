    ...
    public void testAllFixtures() {
        ...
        for (ErrorFixture fixture : FIXTURES) {
            try {
                ...
                failures.add(new TestResult(fixture,
                        new JsonLogicException("Expected an exception at " + fixture.getExpectedJsonPath(), "")));
        ...
        for (TestResult testResult : failures) {
            ...
            System.out.printf("FAIL: %s\n\t%s\n\tExpected: %s at %s Got: \"%s\" at \"%s\"\n%n",
                    fixture.getJson(),
                    fixture.getData(),
                    fixture.getExpectedError(),
                    fixture.getExpectedJsonPath(),
                    exception.getMessage(),
                    exception.getJsonPath());
        }

        Assert.assertEquals(String.format("%d/%d test failures!", failures.size(), FIXTURES.size()),
                0,
                failures.size());
    }
